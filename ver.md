...
- 내용:
  - `web/src/app/page.tsx` 내부 '바로 시작하기', '계산기 바로가기' 버튼의 링크(href)가 잘못된 Hash(`#calculator`)로 되어 있던 것을 실제 라우팅 경로인 `/calculator` 로 변경


## v4.0.26 (2026-07-10)
- 설명: 랜딩 페이지 UI 개편 (요금제 숨김 및 도입효과/작동방식 추가)
- 내용:
  - 요금제(`pricing`) 섹션 및 관련 네비게이션 버튼 숨김 처리
  - 도입 효과(`proof`) 섹션 신설: 계산 시간 단축, 가산세 리스크 0%, 고객 신뢰도 향상 안내 추가
  - 작동 방식(`how`) 섹션 신설: 기본 정보 입력 -> 가액/경비 입력 -> 결과 확인의 3단계 직관적 안내 추가
  - 주요 기능(`features`), 도입 효과(`proof`), 작동 방식(`how`) 간 GNB 스크롤 이동 연동
